...
export async function getPrerenderRoutes(): Promise<string[]> {
  ...
  const dynamic = [
    ...use(bkkkExhibitions, FALLBACK_BKKK_EXHIBITIONS).map(s => `/bk/exhibitions/${s}`),
    ...use(kyafExhibitions, FALLBACK_KYAF_EXHIBITIONS).map(s => `/kyaf/exhibitions/${s}`),
    ...use(bkkkMovingImage, FALLBACK_BKKK_MOVING_IMAGE).map(s => `/bk/moving-image/${s}`),
    ...use(bkkkArtists, FALLBACK_BKKK_ARTISTS).map(s => `/bk/artists/${s}`),
    ...use(kyafArtists, []).map(s => `/kyaf/artists/${s}`),
    ...use(bkkkActivities, FALLBACK_BKKK_ACTIVITIES).map(s => `/bk/activities/${s}`),
